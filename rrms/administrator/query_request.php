...
if (isset($_POST['req_id']) && isset($_POST['remarks'])) {
    $req_id  = mysqli_real_escape_string($conn, $_POST['req_id']);
    $remarks = mysqli_real_escape_string($conn, $_POST['remarks']);
    $dated   = date('Y-m-d H:i:s');

    $sql = "
        INSERT INTO tblremarks (req_id, remarks, dated)
        VALUES ('$req_id', '$remarks', '$dated')
        ON DUPLICATE KEY UPDATE 
            remarks = VALUES(remarks),
            dated = VALUES(dated)
    ";

    echo mysqli_query($conn, $sql) ? 'success' : 'Database error: ' . mysqli_error($conn);
}
                ...
                <?php 
                    ...
                    while ($r = mysqli_fetch_assoc($ruselect)) {
                        ...
                        <?php 
                            ...
                            $hasRemarks = mysqli_num_rows(mysqli_query($conn, "SELECT 1 FROM tblremarks WHERE req_id='$r[req_id]' LIMIT 1")) > 0;
                            $btnClass = $hasRemarks ? 'btn-info' : 'btn-secondary';
echo '

  <button onclick="add_remarks(\''.$r['req_id'].'\')" 
          type="button" class="btn '.$btnClass.' btn-sm" title="Remarks">
...
';
                        ...
                        echo '</td>
                              <td width="1%" class="text-nowrap">
                                  <div class="btn-group" role="group" aria-label="Basic mixed styles example">
                                    <button onclick="showDetailsDrawer(\''.$r['req_id'].'\')" type="button" class="btn btn-primary btn-sm" title="Details"><i class="bi bi-menu-down"></i></button>
                                    <button onclick="editRequestDrawer(\''.$r['req_id'].'\')" type="button" class="btn btn-success btn-sm" title="Edit"><i class="bi bi-pencil-square"></i></button>';

                        // Buttons for non-released requests
                        if ($r['req_datetime_released'] == '0000-00-00 00:00:00' || empty($r['req_datetime_released'])) {
                            echo '
                                <button onclick="request_docs(\''.$r['req_id'].'\', \''.$fullname.'\')" type="button" class="btn btn-warning btn-sm" title="Request"><i class="bi bi-chat-square-dots-fill"></i></button>
                                <button onclick="remove_requestor(\''.$r['req_id'].'\')" type="button" class="btn btn-danger btn-sm" title="Delete"><i class="bi bi-trash-fill"></i></button>';
                        }
